...
const WbnPlayer = props => {
  ...
  const endCallback = () => {
    const videoId = props.match.params.activeVideo;
    const currentVideoIndex = state.video.findIndex(
      video => video.id === videoId
    )

    const nextVideo = currentVideoIndex === state.video.length - 1 ? 0 : currentVideoIndex + 1;
  ...
  const progressCallback = e => {
    if(e.playedSeconds > 10 && e.playedSeconds < 11){
      setState({
        ...state,
        video: state.videos.map( element => {
